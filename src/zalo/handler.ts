      ...
      if (msg.isSelf) {
        ...
        if (isEcho) {
          const echoQuoteData: ZaloQuoteData = {
            ...
            content:    typeof msg.data.content === 'string'
              ? msg.data.content
              : (msg.data.content as Record<string, unknown>),
            ttl:        msg.data.ttl ?? 0,
          ...
          sentMsgStore.attachQuote(
            [msg.data.msgId, msg.data.realMsgId, msg.data.cliMsgId]
...
${escapeHtml(photoCaption)}`
  ...
  api.listener.on('reaction', async (reaction: any) => {
    try {
      ...
      // If empty reaction icon → user removed reaction; keep our remove-summary
      // behavior below, but still resolve target IDs robustly.
