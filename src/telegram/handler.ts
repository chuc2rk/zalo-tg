...
function isSenderOnlyForwardCaption(caption: string | undefined, sourceTopicId: number | undefined): boolean {
  ...
  // labels (e.g. "Nguyên Aeri", optionally prefixed with @chuc2rk for DMs).
  // If such a caption exactly equals a known topic/contact name, treat it as
  // bridge metadata and do not resend it as a real Zalo text message.
  return store.all().some(e => sameAsTopicName(e.name));
}
