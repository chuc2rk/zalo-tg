...
export const sentMsgStore = {
  ...
  /** Also bind the richer Zalo quote data once the self-echo arrives. */
  attachQuote(zaloMsgIds: string[], quote: ZaloQuoteData): void {
    ...
    // message so later replies can resolve rich quote metadata, even after restart.
    for (const id of ids) {
      _sentByZaloId.set(id, tgMsgId);
